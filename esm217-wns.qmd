...
county <- read_sf(here("data", "tl_2023_us_county", "tl_2023_us_county.shp")) %>% 
  mutate(county = NAME)
...
# join them
wns_county <- left_join(county, wns, by = "county") %>% 
  mutate(determination = as.factor(determination))

# Exploratory WNS-by-County Map
map_wns_county <- tm_shape(wns_county, bbox = bbox)+
...
#tmap_save(map_wns_county, here("outputs", #"map_wns_county.png"))
```

![](outputs/map_wns_county.png)
...
# join them
pesticides_county <- left_join(county, pesticides, by = "COUNTYFP") 

pest_test <- pesticides_county %>% 
    filter(YEAR == 2017)

tm_shape(pest_test, bbox = bbox)+
